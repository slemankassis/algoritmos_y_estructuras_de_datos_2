...
bool graph_has_cycle(graph_t g) {
    ...
    elem_t elem = NULL;
    union_find_t uf = union_find_create(graph_vertices_count(g));

    bool b = false;
    uint l = 0;
    uint r = 0;

    for (unsigned int i = 0; i < graph_edges_count(g); i++) {

        l = union_find_find(uf, vertex_label(edge_left_vertex(edges[i])));
        r = union_find_find(uf, vertex_label(edge_right_vertex(edges[i])));
