...
bool pair_is_equal(pair_t pair, pair_t other){
	...
	return (index_is_equal(pair_fst(pair), pair_fst(other)) && data_is_equal(pair_snd(pair), pair_snd(other)));
}
...
pair_t pair_copy(pair_t pair){
	...
	pair_t other = NULL;
	other = pair_from_index_data(index_copy(pair_fst(pair)), data_copy(pair_snd(pair)));
